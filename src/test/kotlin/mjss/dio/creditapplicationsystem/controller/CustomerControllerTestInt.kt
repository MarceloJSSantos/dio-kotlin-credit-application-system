...
class CustomerControllerTestInt{
    @Autowired
    ...
    fun `should create a customer and return 201 status`(){
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(URL)
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("Marcelo"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("Santos"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("53735495761"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("marcelo.santos@email.com.br"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("1000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("11222333"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("Rua Projetada A"))
    ...
    @Test
    fun `should not save a customer with firstNme empty and return 400 status`(){
        //given
    ...
    fun `should not find customer with invalid id and return 400 status`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$.exception")
                    .value("class mjss.dio.creditapplicationsystem.exception.BusinessException")
            )
            .andExpect(MockMvcResultMatchers.jsonPath("$.details[*]").isNotEmpty)
            .andDo(MockMvcResultHandlers.print())
    }
    ...
    fun `should update a customer and return 200 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("$URL?customerId=${customer.id}")
            .contentType(MediaType.APPLICATION_JSON).content(valueAsString))
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("Name Update"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("Last Update"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("53735495761"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("marcelo.santos@email.com.br"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("15000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("22444666"))
    ...
    private fun builderCustomerDto(
        ...
        cpf: String = "53735495761",
        email: String = "marcelo.santos@email.com.br",
        income: BigDecimal = BigDecimal.valueOf(1000.0),
        password: String = "1234",
        zipCode: String = "11222333",
        street: String = "Rua Projetada A"
    ) = CustomerDto (
        firstName = firstName,
        lastName = lastName,
        cpf = cpf,
